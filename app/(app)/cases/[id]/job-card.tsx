...
const partHeadCls = "eyebrow px-1.5 pb-1 text-left font-normal";
const partCellCls = "px-1.5 py-1";

/**
 * The five editable cells of a part line, shared by the edit row and by the
 * entry row that always sits at the bottom of the table.
 *
    ...
    <li className="border-b last:border-b-0">
      ...
      {expanded && (
        <div className="flex flex-col gap-3 border-t border-dashed bg-surface-2/40 px-3.5 py-3">
          ...
          {/* Part lines as a table with an entry row pinned to the bottom.
              Founder call after M5: adding a part is the one thing on this
              card that must never be something you open first, so the row is
              structural rather than a state someone can toggle away. */}
          <div className="flex flex-col gap-1.5">
            <span className="eyebrow">{t("parts.title")}</span>
            {readOnly && job.partLines.length === 0 ? (
                  ...
                  <tbody>
                    {job.partLines.map((line) =>
                      editingPart === line.id ? (
                        ...
                        <tr key={line.id} className="border-b border-dashed hover:bg-surface-2">
                          <td className={partCellCls}>
                            {line.name}
            ...
            {!readOnly && (
              <>
                <form id={addPartFormId} ref={addPartFormRef} action={handleAddPart} />
                {editingLine && (
                  <form
                    id={editPartFormId}
                    action={(formData) => {
                      ...
                      void run(() =>
                        updatePartLine(lineId, {
                          ...
                          orderStatus: String(
                            formData.get("orderStatus") ?? editingLine.orderStatus,
                          ),
                        }),
                      );
                    }}
