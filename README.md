...
Each Shop connects its **own** LINE Official Account in `/settings` (ADR-002) — credentials verified against LINE, then stored encrypted (ADR-004) — and the OA's webhook captures the LINE identities staff match to Customers by hand (ADR-005). On a case, the **Customer timeline** sits beside the internal one: a Thai draft is pre-filled from the case's real Jobs in customer-safe wording, staff edit it, attach up to four case photos, preview exactly what will go out, and press send. Every send is an immutable record of what the customer actually saw, and appears on the internal timeline as an event. Nothing is sent automatically *yet* — that changes in M7.10 ([ADR-007](docs/adr/ADR-007-system-sent-customer-updates.md) supersedes ADR-003): milestone messages ride on check-in, Ready and Delivered, and progress notices go out on their own for a closed list of status changes. Setting up an Official Account is walked through step by step in [docs/LINE-SETUP.md](docs/LINE-SETUP.md).
